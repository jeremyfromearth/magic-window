...
void magicwindow::app::draw() {
  ...
  cfg::window * data = window->getUserData<cfg::window>();
...
void magicwindow::app::magic() {
  ...
  params_window->setUserData(new cfg::window(-1, Rectf(), vec2()));
  ...
  if (ctx.config.get_window_mode() == cfg::window::DISPLAY_CUSTOM) {
  ...
  if (ctx.config.get_window_mode() == cfg::window::DISPLAY_GRID) {
    ...
    for(int r = 0; r < rows; r++) {
      for(int c = 0; c < cols; c++) {
        ...
        window->setUserData(new cfg::window(index, Rectf(x, y, w, h), vec2(-x, -y)));
        if(ctx.config.is_fullscreen()) window->setFullScreen();
